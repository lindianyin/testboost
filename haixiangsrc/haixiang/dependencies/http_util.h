...
inline string CHttpParamStringMaker::get_paramswithout()
{
	if(!m_strParamLines.empty())
		return m_strParamLines;
...
inline int CHttpLengthAnaly::get_length_ex(const char* szData, int nDataLen, int& nContentPos, int& nContentLen)
{
	...
	if(bPostType || bRespType)
	{
		...
		{
			char* pContentLenEnd = strstr(pHeadEnd + 4, "\r\n0\r\n\r\n");
			if(pContentLenEnd - szData <= nDataLen)
			{
				nContentLen = pContentLenEnd - pHeadEnd - 4 + 1 + 6;
			}
		}
	}
...
public:
	...
	bool parseJson(const char* szHttpParam, int nLen);
...
inline bool CHttpParamParser::parseJson(const char* szHttpParam, int nLen)
{
	//清理内存
	m_mapValues.clear();
	char* httpParam = (char*)szHttpParam;
	httpParam = strstr(httpParam, "{");
	if(szHttpParam + nLen < httpParam)
	{
		return true;
	}
	...
	memcpy(buf.buf, httpParam + 1, nLen - (httpParam - szHttpParam) - 1);
	...
	while(szParam <= buf.buf + nLen - (httpParam - szHttpParam) - 1)
	{
		if(szParam[0] == '\0')
			break;
		...
		if(strKey == "jinbi")
		{
			memcpy(bufKeyValue.buf, szValue + 2, szSplit-szValue - 2);
		}
		else
		{
			memcpy(bufKeyValue.buf, szValue+3, szSplit-szValue - 3);
		}
...
inline int CHttpParser::parse(const char* szHttpReq, int nDataLen, int nExtraParamType)
{
	...
	else if(m_pszContent)
	{
		if(nExtraParamType == HTTP_UTIL_PARAM_ALL || nExtraParamType == HTTP_UTIL_PARAM_CONTENT)
		{
			if(nContentPos == 0)
			{
				m_paramParser.parseJson(m_pszContent, m_nContentLen);
			}
			else
			{
