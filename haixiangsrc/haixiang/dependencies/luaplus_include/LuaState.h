...
#define LUAFUNCTION_POSTCALL(numArgs, nRets) \
		if (lua_pcall(L, numArgs, nRets, 0)) \
		{ \
		...
		{\
			result_v.push_back(LPCD::Get(LPCD::TypeWrapper<RT>(), L, -1 - i));\
		}



//modified by hjt for support multi-returns
template <class RT, int rets = 1>
class LuaFunction
{
public:
	std::vector<RT> result_v;
	...
	void operator()(P1 p1, P2 p2, P3 p3, P4 p4, P5 p5, P6 p6, P7 p7)
	{
		...
		LPCD::Push(L, p6);
		LPCD::Push(L, p7);
		LUAFUNCTION_POSTCALL(7, rets);
	}
	inline bool		IsNil() {return m_functionObj.IsNil();}
protected:
