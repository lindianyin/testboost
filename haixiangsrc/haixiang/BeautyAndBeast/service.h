...
{
public:
	longlong			min_gain;			//区间最小值
	longlong			max_gain;			//区间最大值
	unsigned int	ratio;				//抽水比例（万份之一_1%%）
};
...
public:
	...
	longlong	broadcase_gold;				//喇叭配置(金币要求)
	longlong	bet_cap_;
	...
	vector<tax_turn> tax_turn_;			//每轮赢家抽水

	/******************************计划任务******************************/
	//周长计划任务
	int weeklyTask_week;
	std::string weeklyTask_time;
...
public:
	...
	void							run_scheduled_task(const boost::system::error_code&);						//启动时的计划任务定时器回调函数
	...
	boost::shared_ptr<preset_riches> riches;
};
