...
enum
{
	...
	GET_CLSID(msg_banker_ranking_ret) = 23,
};
...
class msg_banker_ranking_ret : public msg_base<none_socket>
{
public:
	char			uname_[max_name];
	double		rank_val;
	short			idx;
	short			type;	//0是结束， 1是开始， 2是正常
	msg_banker_ranking_ret()
	{
		head_.cmd_ = GET_CLSID(msg_banker_ranking_ret);
		memset(uname_, 0, max_name);
		rank_val = 0;
		idx = 0;
		type = 0;
	}
	...
	int write(char*& data_s, unsigned int l)
	{
		...
		write_data(rank_val, data_s);
		write_data(idx, data_s);
