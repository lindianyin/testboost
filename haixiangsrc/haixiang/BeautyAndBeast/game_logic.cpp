...
void beatuy_beast_logic::random_result()
{
	...
	for (int i = 0 ; i < bots_.size(); i++)
	{
		if (std::find(banker_applications_.begin(), banker_applications_.end(), bots_[i]->uid_) == banker_applications_.end() && 
			...
			if(the_service.cache_helper_.get_currency(bots_[i]->uid_) <= cr){
				if(the_service.cache_helper_.give_currency(
					bots_[i]->uid_,
					rand_r(the_service.the_config_.banker_deposit_, 3 * the_service.the_config_.banker_deposit_),
					cr) == ERROR_SUCCESS_0){
				}
	...
	if (the_banker_.get() && !the_banker_->is_bot_){
		cheat_win = 0;
		float rate = the_service.get_personal_rate(the_banker_);
		//如果不在白名单下,不调整cheat_win
		if (abs(rate - 1.0) > 0.001){
			//如果要打庄
			if (rate < 1.0)	{
				//赔最大
				if (rand_r(10000) > (rate * 10000)){
					cheat_win = -1;
				}
			}
			//如果要利庄
				...
				//赔最小
				if (rand_r(10000) < ((rate - 1.0) * 10000))	{
					cheat_win = 1;
				}
			}
		}
...
int beatuy_beast_logic::apply_cancel_banker( player_ptr pp )
{
	...
	if (pp->banker_turns_ < the_service.the_config_.min_banker_turn_ && !is_no_pour)
...
// 		}
	...
	if(the_service.cache_helper_.apply_cost(pp->uid_, this_time_bet, out_count) == ERROR_SUCCESS_0){
		...
		if (!pp->is_bot_){
			...
			SET_FINAL_VALUE("game_id", strid_);
			EXECUTE_IMMEDIATE();
		}
...
void beatuy_beast_logic::balance_result()
{
	...
	//添加押注总金额，机器人不统计,之所以不在下注时加入库存是不想让下注的额度影响库存进而影响roll结果。
	//要统计系统输赢，只需要立足于玩家,玩家赢钱，等于系统输钱，玩家输钱，等价于系统赢钱。
	//所以不用区分是否玩家做庄。
	//玩家做庄->赢玩家的钱: 玩家输了钱，库存加上去，庄家赢了钱，系统库存减下来.最终，库存增减量为0
	//       ->赢机器人的钱:没有玩家输了钱，库存没变化,庄家赢了钱，系统库存减下来.最终，库存增量为负.
	//系统做庄->赢玩家的钱: 玩家输了钱，库存加上去。
	//			 ->赢机器人的钱:没有玩家输了钱，库存没变化.

	longlong sys_get_price = the_service.add_stock(0);
	...
	if (this_turn_result_.pid_ == 100){
		for (int i = 0; i < bets_.size(); i++)
		{
			...
			if( the_service.the_config_.is_pid(bi.present_id_) ){
				...
				total_bets += bi.bet_count_;
				//记录库存变化
				player_ptr pp = the_service.get_player(bi.uid_);
				if (pp.get() && !pp->is_bot_){
					the_service.add_stock(bi.bet_count_);
				}
			}
		}
	}
	...
	if(the_banker_.get()){
		if(banker_win > 0){
			//获得系统扣税
			longlong tax1 = banker_win  / 100.0 * the_service.the_config_.player_banker_tax_;
			//扣取赢家税
			longlong tax2 = 0;
			vector<tax_turn>::iterator iter;  
			for (iter = the_service.the_config_.tax_turn_.begin(); iter != the_service.the_config_.tax_turn_.end(); iter++){  
				if(banker_win >= (*iter).min_gain && banker_win <= (*iter).max_gain){
					tax2 = ceil(banker_win / 10000.0 * (*iter).ratio);
					break;
				}
			} 
			this_time_tax += (tax1 + tax2);
			banker_deposit_ -= (tax1 + tax2);
			banker_win -= (tax1 + tax2);
		}
		...
		if (!the_banker_->is_bot_){
			the_service.add_stock(-banker_win);
		}
		...
		if(!the_banker_->is_bot_)
		{
			...
			SET_FIELD_VALUE("game_id", strid_);
			SET_FIELD_VALUE("uid", the_banker_->uid_);
			SET_FIELD_VALUE("turn", turn_);
	...
	{
		game_report_data& grd = it_rpt->second;		
		it_rpt++;
		...
		if(pp.get()){
			...
			if(grd.actual_win_ - grd.pay_ > 0)
			{
				for (iter = the_service.the_config_.tax_turn_.begin(); iter != the_service.the_config_.tax_turn_.end(); iter++)  
				{ 
					if((grd.actual_win_ - grd.pay_) >= (*iter).min_gain && (grd.actual_win_ - grd.pay_) <= (*iter).max_gain) 
					{
						tax1 = ceil((grd.actual_win_ - grd.pay_) / 10000.0 * (*iter).ratio);
					}

					if((grd.should_win_ - grd.pay_) >= (*iter).min_gain && (grd.should_win_ - grd.pay_) <= (*iter).max_gain)
					{
						tax2 = ceil((grd.should_win_ - grd.pay_) / 10000.0 * (*iter).ratio);
					}
				}  
			}

			longlong tax = grd.actual_win_  / 100.0 * the_service.the_config_.player_tax2_;
			grd.actual_win_ -= (tax + tax1);
			longlong tax_ = grd.should_win_  / 100.0 * the_service.the_config_.player_tax2_;
			grd.should_win_ -= (tax_ + tax2);
			this_time_tax += (tax + tax1);
		}
		...
		if (pp.get() && !pp->is_bot_)	{
			...
			msg.banker_win = banker_win;
			pp->send_msg_to_client(msg);

			//记录库存变化,玩家赢=系统输
			the_service.add_stock(-grd.actual_win_);
			//发送广播消息给7158
	...
	if(the_banker_.get())
	{
		player_ptr pp = get_player(the_banker_->uid_);
		if(pp.get() && !pp->is_bot_)
		{
			//获得当前系统时间
			//date date1 = boost::gregorian::day_clock::local_day();

			std::map<std::string, riche_info>::iterator it = the_service.riches->list.find(the_banker_->uid_);
			if(it != the_service.riches->list.end())
			{
				player_ptr pp = get_player(the_banker_->uid_);
				it->second.uname_ = pp->name_;
				it->second.val_ += total_bets;
			}else{
				riche_info _info;
				_info.uid_ = the_banker_->uid_;
				player_ptr pp = get_player(the_banker_->uid_);
				_info.uname_ = pp->name_;
				_info.val_ = total_bets;
				the_service.riches->list.insert(pair<std::string, riche_info>(the_banker_->uid_, _info));
			}
		}
	}
	
	{
		...
		if(the_banker_.get())
		{
			SET_FIELD_VALUE("banker_uid", the_banker_->uid_);
		}else
		{
			SET_FIELD_VALUE("banker_uid", "");
		}
		...
		SET_FIELD_VALUE("total_bets", total_bets);
		SET_FIELD_VALUE("total_sys_win", the_service.add_stock(0) - sys_get_price);
...
int beatuy_beast_logic::promote_to_banker( player_ptr pp )
{
	...
	if(pp.get()){
		...
		msg.banker_turn = pp->banker_turns_;
		broadcast_msg(msg);
...
{
	{
		...
		msg.banker_deposit = the_service.the_config_.banker_deposit_;
	...
	if (!the_banker_.get()){
		...
		msg.banker_turn = the_banker_->banker_turns_;
	}	
	pp->send_msg_to_client(msg);
...
{
	//发送喇叭到7158平台，根据不同的游戏配置不同
	if(the_service.the_config_.game_name == "beauty" ||
		the_service.the_config_.game_name == "fruit" ||
		the_service.the_config_.game_name == "games" ||
		the_service.the_config_.game_name == "motors")
	{
		//单注用户奖励≥100万广播
		if(report_.actual_win_ >= the_service.the_config_.broadcase_gold)
		{
			...
			//内容格式:赢得了%lld金币奖励，........!
			std::map<std::string, std::string>::iterator it = the_service.the_config_.broadcast_telfee.find(the_service.the_config_.game_name);
			if(it == the_service.the_config_.broadcast_telfee.end())
			{
				glb_log.write_log("Broadcast Text config Not Find Text");
			}else{
				sprintf_s(buff, 512, it->second.c_str(), report_.actual_win_);
...
int task_clear_riches::routine()
{
	the_service.riches->list.clear();
