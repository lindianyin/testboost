...
{
    TIXML_SNPRINTF( buffer, "%d", v );
}


void XMLUtil::ToStr( unsigned v, char* buffer, int bufferSize )
{
    TIXML_SNPRINTF( buffer, "%u", v );
}


void XMLUtil::ToStr( bool v, char* buffer, int bufferSize )
{
    TIXML_SNPRINTF( buffer, "%d", v ? 1 : 0 );
}


void XMLUtil::ToStr( float v, char* buffer, int bufferSize )
{
    TIXML_SNPRINTF( buffer, "%g", v );
}
...
void XMLDocument::PrintError() const
{
    if ( _errorID ) {
        ...
        if ( _errorStr1 ) {
            TIXML_SNPRINTF( buf1, "%s", _errorStr1 );
        }
        if ( _errorStr2 ) {
            TIXML_SNPRINTF( buf2, "%s", _errorStr2 );
        }
