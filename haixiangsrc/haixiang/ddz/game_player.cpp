...
int	ddz_player::update()
{
	return ERROR_SUCCESS_0;
}

void ddz_player::reset_temp_data()
{
	turn_cards_.clear();
	is_win_ = -1;
	bet_ = 0;
	actual_win_ = 0;
	//emOpenDealType_ = emOpenDeal_Null;
}

//在连接断掉的时候调用
void ddz_player::on_connection_lost()
{
...
void ddz_player::win_bet( longlong v, player_ptr banker )
{
	if (v < 0) return;
	actual_win_ += v;
...
void ddz_player::lose_bet( longlong v, player_ptr banker )
{
	if (v < 0) return;
	actual_win_ -= v;
...
int ddz_player::guarantee( longlong v, bool take_all, bool sync_to_client)
{
	if (v < 0) return 5;
...
bool ddz_player::has_card( ddz_card& c )
{
