...
int ddz_service::on_run()
{
	...
	pmc->iid_ = 5;
	pmc->time_type_ = 0;
	pmc->register_time_ = 120000;
	pmc->enter_time_ = 60000;
	pmc->matching_time_ = 4 * 60000;
	pmc->scene_set_ = sc;
	pmc->prizes_.insert(pmc->prizes_.begin(), match_prize[1], (match_prize[1] + 7));
	...
	pmc->iid_ = 10;
	pmc->time_type_ = 0;
	pmc->register_time_ = 3000000;	//50分钟
	pmc->enter_time_ = 60000;
	...
	for (auto itm : the_matches_)
	{
		itm.second->enter_wait_register_state();
	}
#endif

// 	logic_ptr plogic(new ddz_logic(0));
// 	plogic->scene_set_ = sc;
// 	plogic->start_logic();
// 	the_service.the_golden_games_.push_back(plogic);
	return ERROR_SUCCESS_0;
}

void ddz_service::player_login( player_ptr pp )
{

}

ddz_service::ddz_service():game_service_base<service_config, ddz_player, remote_socket<ddz_player>, logic_ptr, match_ptr, scene_set>(IPHONE_NIUINIU)
{
	wait_for_config_update_ = false;
}

void ddz_service::on_main_thread_update()
{
	//更新配置
...
void ddz_service::on_sub_thread_update()
{
}

void ddz_service::save_currency_to_local( string uid, longlong val, int is_free )
{
	if (is_free){
		uid += "_FREE";
	}
...
void ddz_service::delete_from_local( string uid, int is_free )
{
	if (is_free){
		uid += "_FREE";
	}
...
int ddz_service::restore_account( player_ptr pp, int is_free )
{
...
player_ptr ddz_service::pop_onebot(logic_ptr plogic)
{
	player_ptr pp;
	longlong c = 0;
	if (!pp.get() && !g_bot_names.empty()){
		pp.reset(new ddz_player);
		pp->is_bot_ = 1;
...
ddz_service::msg_ptr ddz_service::create_msg( unsigned short cmd )
{
	DECLARE_MSG_CREATE()
	...
	REGISTER_CLS_CREATE(msg_play_cards);
	...
	REGISTER_CLS_CREATE(msg_play_cards_tips);//提示
	REGISTER_CLS_CREATE(msg_play_open_deal);//明牌
	REGISTER_CLS_CREATE(msg_play_auto);			//托管

	END_MSG_CREATE()
	return ret_ptr;
}

int ddz_service::handle_msg( msg_ptr msg, remote_socket_ptr from_sock )
{
...
match_ptr ddz_service::create_match(int tpid, int insid)
{
	//内置比赛场
	if (tpid == 1){
...
int __thiscall msg_enter_match_game_req::handle_this(void){return 0;}
int __thiscall msg_query_match_req::handle_this(void){return 0;}
int __thiscall msg_register_match_req::handle_this(void){return 0;}
int __thiscall msg_agree_to_join_match::handle_this(void){return 0;}